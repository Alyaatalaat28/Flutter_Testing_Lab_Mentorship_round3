...
void main() {
  ...
  group('إضافة العناصر للعربة (Add Items)', () {
  ...
  group('حذف العناصر من العربة (Remove Items)', () {
  ...
  group('حسابات الأسعار والخصومات (Price Calculations)', () {
  ...
  // ==================== المجموعة الرابعة: Edge Cases (الحالات الخاصة) ====================
  group('اختبار الحالات الخاصة (Edge Cases)', () {
    
    // ==================== اختبار 12: العربة الفاضية ====================
    testWidgets('يجب عرض رسالة "Cart is empty" للعربة الفاضية', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ShoppingCart(),
      ...
      // نتحقق من الحالة الابتدائية
      expect(find.text('Cart is empty'), findsOneWidget);
      expect(find.text('Total Items: 0'), findsOneWidget);
      expect(find.textContaining('Subtotal: \$0.00'), findsOneWidget);
      expect(find.textContaining('Total Discount: \$0.00'), findsOneWidget);
      expect(find.textContaining('Total Amount: \$0.00'), findsOneWidget);
    });

    // ==================== اختبار 13: منتج بدون خصم ====================
    testWidgets('يجب التعامل مع منتج بدون خصم', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ShoppingCart(),
      ...
      // نضيف iPad (مفيش عليه خصم)
      await tester.tap(find.text('Add iPad'));
      await tester.pump();

      // السعر = 1099.99 بدون خصم
      expect(find.textContaining('Subtotal: \$1099.99'), findsOneWidget);
      expect(find.textContaining('Total Discount: \$0.00'), findsOneWidget);
      expect(find.textContaining('Total Amount: \$1099.99'), findsOneWidget);

      // لو دورنا على نص "Discount:" مش هنلاقيه لأن مفيش خصم
      expect(find.textContaining('Discount: 0%'), findsNothing);
    });

    // ==================== اختبار 14: عرض نسبة الخصم بشكل صحيح ====================
    testWidgets('يجب عرض نسبة الخصم بشكل صحيح', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ShoppingCart(),
    ...
    // ==================== اختبار 15: حساب Item Total لكل منتج ====================
    testWidgets('يجب حساب Item Total لكل منتج بشكل صحيح', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ShoppingCart(),
      ...
      // Item Total = (999.99 * 1) - (999.99 * 0.1 * 1) = 999.99 - 99.999 = 899.99
      expect(find.textContaining('Item Total: \$899.99'), findsOneWidget);
    });

    // ==================== اختبار 16: التعامل مع أرقام عشرية ====================
    testWidgets('يجب التعامل مع الأرقام العشرية بدقة', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ShoppingCart(),
      ...
      // نتأكد من التنسيق الصحيح للأرقام (رقمين بعد العلامة العشرية)
      expect(find.textContaining('Price: \$999.99 each'), findsOneWidget);
      expect(find.textContaining('Item Total: \$899.99'), findsOneWidget);
    });

    // ==================== اختبار 17: إضافة كميات كبيرة ====================
    testWidgets('يجب التعامل مع كميات كبيرة', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ShoppingCart(),
    ...
    // ==================== اختبار 18: مسح العربة ثم إضافة منتجات جديدة ====================
    testWidgets('يجب إضافة منتجات بعد مسح العربة', (WidgetTester tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: ShoppingCart(),
  ...
  group('اختبار عناصر الواجهة (UI Elements)', () {
